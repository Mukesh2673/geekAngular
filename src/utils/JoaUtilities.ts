...
export class JoaUtilities {
  ...
  /**
   * Verifiy token is valid or not
   * @param {string} token
   */
  public static getUserDetail = async (token: string) => {
    return new Promise(function (resolve, reject) {
      jwt.verify(token, config.get("JWT_SECRET_KEY"), function (error, result) {
        if (error) {
          return reject(error);
        } else {
          return resolve(result);
   ...
   */
  public static sendEmail = async (data: any) => {
    return new Promise(function (resolve, reject) {
