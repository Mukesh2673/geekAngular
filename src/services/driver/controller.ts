...
export const getDriverData = async (skip: any, limit: any) => {
    try {
        skip = parseInt(skip);
        limit = parseInt(limit);
        var totalCount
        userModel.countDocuments({ role: ['Driver'] }, function (err, count) {
            totalCount = count
        });


        let item: any = await userModel.find({ role: ['Driver'] }).limit(limit).skip(skip);
        let data = {
            item: item,
            totals: totalCount,
            status: 'success'
