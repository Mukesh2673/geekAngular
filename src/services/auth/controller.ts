...
export const login = async (req: any, res: any, next: any) => {
  try {
    ...
    user.save(user);
    //response.token(res, userToken, 200);
    ...
    return JoaUtilities.sendResponsData({ code: 200, message: 'Success', data: result });
...
export const logout = async (token: any) => {
  ...
  if (userRes) {
    userRes.accessToken = "";
    await userRes.save();
    return JoaUtilities.sendResponsData({ code: 200, message: "Success" });
  } else {
    throw new HTTP400Error(
      JoaUtilities.sendResponsData({
        code: 400,
 ...
 */
 export const changePassword = async (token: any, currentPassword: string, password: string) => {
  ...
  if (userRes) {
    const match = await JoaUtilities.VerifyPassword(currentPassword, userRes.password);
    if (match) {
      let hashedPassword = await JoaUtilities.cryptPassword(password);
      userRes.password = hashedPassword;
      userRes.save();
      return JoaUtilities.sendResponsData({ code: 200, message: 'Success', data: userRes });
    } else {
      throw new HTTP400Error(JoaUtilities.sendResponsData({ code: 400, message: config.get('ERRORS.USER_ERRORS.INVALID_CURRENT_PASSWORD') }));
    }
  } else {
    throw new HTTP400Error(JoaUtilities.sendResponsData({ code: 400, message: config.get('ERRORS.USER_ERRORS.USER_NOT_EXIST') }));
  }
}


export const forgotPassword = async (body: any) => {
  let userRes: any = await userModel.findOne({ email: body.email });
  if (userRes) {
    userRes.email = body.email;
    let randomCode = JoaUtilities.genAlphaNumericCode(15);
    userRes.resetPasswordToken = randomCode;
    await userRes.save();
    randomCode
    let link = `${config.get("SITE_URL")}/auth/reset-password/${randomCode}`;
    let message = `<p>Hi,</p>
...
export const resetPassword = async (id: string, body: any) => {
  let userRes: any = await userModel.findOne({ resetPasswordToken: id });
  if (userRes) {
    let hashedPassword = await JoaUtilities.cryptPassword(body.password);
    userRes.password = hashedPassword;
...
export const test = async (a: any, b: any, c: any) => {
  return `<html>
  <head>
